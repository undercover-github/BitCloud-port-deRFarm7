...
******************************************************************************/
const uint8_t halAdcDivider[7]   = {2, 3, 4, 7, 15, 31, 63};
...
void halOpenAdc(void)
{
  ...
  if (RESOLUTION_8_BIT == adcDesc->resolution)
    ...
    AT91C_BASE_ADC->ADC_MR = ((uint32_t)halAdcDivider[adcDesc->sampleRate] << 8) |
                             ((uint32_t)halAdcHoldTick[adcDesc->sampleRate] << 24) |
                             ((uint32_t)halAdcStartTick[adcDesc->sampleRate] << 16) |
                             AT91C_ADC_LOWRES;
  else
...
void halStartAdc(HAL_AdcChannelNumber_t channel)
{
  ...
  AT91C_BASE_ADC->ADC_RCR = adcDesc->selectionsAmount;
  AT91C_BASE_ADC->ADC_RPR = (uint32_t)(adcDesc->bufferPointer);
