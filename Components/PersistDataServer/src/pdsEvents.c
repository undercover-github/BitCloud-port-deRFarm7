...
static EventToMemoryIdMapping_t PROGMEM_DECLARE(pdsMemoryMap[]) =
{
  ...
  EVENT_TO_MEM_ID_MAPPING(BC_EVENT_PERMISSION_TABLE_UPDATED,    CS_TC_PERMISSION_TABLE_MEM_ID),
...
void PDS_StoreByEvents(PDS_MemId_t memoryId)
{
  ...
  PDS_MEM_MASK_CLEAR_ALL(reqMemMask);
  PDS_MEM_MASK_CLEAR_ALL(eventMemMask);
  PDS_MEM_MASK_SET_BIT(reqMemMask, memoryId);
  pdsExpandMemoryMask(reqMemMask);
  ...
  for (uint8_t i = 0U; i < ARRAY_SIZE(pdsMemoryMap); i++)
  {
    ...
    pdsExpandMemoryMask(eventMemMask);
